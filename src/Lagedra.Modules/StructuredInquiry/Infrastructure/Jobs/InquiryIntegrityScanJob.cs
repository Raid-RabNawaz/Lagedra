    ...
    ILogger<InquiryIntegrityScanJob> logger) : IJob
{
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        foreach (var session in openSessions)
        {
            ...
            {
                anomalies++;
                LogStaleSession(logger, session.Id, session.DealId, session.ListingId, unanswered);
            }
        }
    ...
    [LoggerMessage(Level = LogLevel.Warning, Message = "Stale open inquiry session {SessionId} (Deal {DealId}, Listing {ListingId}) — {UnansweredCount} unanswered questions, open > 30 days")]
    private static partial void LogStaleSession(ILogger logger, Guid sessionId, Guid? dealId, Guid listingId, int unansweredCount);
