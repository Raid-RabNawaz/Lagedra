...
public sealed record WithdrawAcceptedInquiryOfferCommand(
    Guid SessionId,
    Guid CallerUserId,
    Guid? OfferId = null) : IRequest<Result<InquiryOfferDto>>;

public sealed class WithdrawAcceptedInquiryOfferCommandHandler(
    InquiryDbContext dbContext,
    IListingProvider listingProvider,
    IDealApplicationStatusProvider dealStatusProvider)
    : IRequestHandler<WithdrawAcceptedInquiryOfferCommand, Result<InquiryOfferDto>>
{
    public async Task<Result<InquiryOfferDto>> Handle(
        WithdrawAcceptedInquiryOfferCommand request,
        ...
        if (!isParticipant)
        {
            return Result<InquiryOfferDto>.Failure(
                new Error("Inquiry.Forbidden",
                    "Only the tenant or host on this thread can withdraw an offer."));
        }

        try
        {
            var offer = session.WithdrawOffer(request.OfferId, request.CallerUserId);
