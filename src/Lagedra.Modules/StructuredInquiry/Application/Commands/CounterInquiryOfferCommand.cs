...
public sealed record CounterInquiryOfferCommand(
    Guid SessionId,
    Guid OfferId,
    Guid CallerUserId,
    long RentCents,
        ...
        else
        {
            return Result<InquiryOfferDto>.Failure(
                new Error("Inquiry.Forbidden",
                    "Only the tenant or host on this thread can counter an offer."));
        }

        try
        {
            var offer = session.CounterOffer(
                request.OfferId,
