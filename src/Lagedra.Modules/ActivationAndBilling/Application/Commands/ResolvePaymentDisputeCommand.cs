...
public sealed record ResolvePaymentDisputeCommand(
    Guid DealId,
    bool PaymentValid,
    Guid OpsUserId) : IRequest<Result<PaymentConfirmationDto>>;

public sealed class ResolvePaymentDisputeCommandHandler(
    BillingDbContext dbContext,
    IClock clock)
    : IRequestHandler<ResolvePaymentDisputeCommand, Result<PaymentConfirmationDto>>
{
    public async Task<Result<PaymentConfirmationDto>> Handle(
        ResolvePaymentDisputeCommand request,
        CancellationToken cancellationToken)
    {
        ...
        confirmation.ResolveDispute(request.PaymentValid, request.OpsUserId, clock);
