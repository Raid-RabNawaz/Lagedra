...
public sealed record PartiallyApproveDamageClaimCommand(
    Guid DealId,
    Guid ClaimId,
    long ApprovedAmountCents,
    string? Notes) : IRequest<Result<DamageClaimDto>>;

public sealed class PartiallyApproveDamageClaimCommandHandler(BillingDbContext dbContext)
    : IRequestHandler<PartiallyApproveDamageClaimCommand, Result<DamageClaimDto>>
{
    public async Task<Result<DamageClaimDto>> Handle(
        PartiallyApproveDamageClaimCommand request, CancellationToken cancellationToken)
    {
        ...
        claim.PartiallyApprove(request.ApprovedAmountCents, request.Notes);
