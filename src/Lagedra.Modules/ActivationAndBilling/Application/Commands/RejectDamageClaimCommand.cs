...
public sealed record RejectDamageClaimCommand(
    Guid DealId,
    Guid ClaimId,
    string? Notes) : IRequest<Result<DamageClaimDto>>;

public sealed class RejectDamageClaimCommandHandler(BillingDbContext dbContext)
    : IRequestHandler<RejectDamageClaimCommand, Result<DamageClaimDto>>
{
    public async Task<Result<DamageClaimDto>> Handle(
        RejectDamageClaimCommand request, CancellationToken cancellationToken)
    {
