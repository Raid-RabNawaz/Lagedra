...
public sealed record RecordInsuranceUnknownCommand(
    Guid DealId) : IRequest<Result<InsuranceStatusDto>>;

public sealed class RecordInsuranceUnknownCommandHandler(
    InsuranceDbContext dbContext)
    : IRequestHandler<RecordInsuranceUnknownCommand, Result<InsuranceStatusDto>>
{
    public async Task<Result<InsuranceStatusDto>> Handle(
        RecordInsuranceUnknownCommand request,
