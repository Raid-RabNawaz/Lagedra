-- Quartz.NET 3.8.1 job-store schema for PostgreSQL (create-only, idempotent).
-- Executed automatically by QuartzBootstrapService when qrtz_job_details is
-- missing, so a fresh database can never crash-loop the worker again.
